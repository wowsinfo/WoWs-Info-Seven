...
class Consumable extends WikiItem {
  ...
  String get priceString =>
      isGold ? priceGold.toString() : priceCredit.toString();
  bool get isGold => priceGold > 0;
  String get profileString =>
      profile.values.map((e) => e.description).join('\n');

  Consumable.fromJson(Map<String, dynamic> json) {
    this.profile = (json['profile'] as Map)
        .map((a, b) => MapEntry(a, Profile.fromJson(b)));
    this.name = json['name'];
  ...
  int compareTo(Consumable other) {
    if (priceCredit == other.priceCredit)
      return consumableId.compareTo(other.consumableId);
            ...
            text: TextSpan(
                ...
                children: [
                  TextSpan(
                      text: priceString + '\n',
                      style: TextStyle(
                          color: isGold ? Colors.orange : Colors.grey)),
                  TextSpan(
                      text: description + '\n\n',
                      style: TextStyle(fontSize: 14)),
                  TextSpan(text: profileString, style: TextStyle(fontSize: 12)),
                ]),
          ),
