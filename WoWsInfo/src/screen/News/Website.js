...
renderWebsite = (data) => {
  return data.map(function(info, index) {    
    return info.map(function(info, index) {
      ...
      if (index == 0) return <Text key={info} style={{padding: 8, backgroundColor: getTextColour(theme[500]), color: theme[500] == '#000000' ? Blue : theme[500], fontWeight: 'bold'}}>{info}</Text>
